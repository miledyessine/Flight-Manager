...
TCHAR szClassName[] = _T("Gestion d'un aeroport");
                   ...
                   int nCmdShow)
{
    ...
    hwnd = CreateWindowEx(
        ...
        _T("Gestion d'un aeroport"), /* Title Text */
...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case 42:
            if (fich_vide())
                MessageBox(hwnd, " Essayer de faire la creation\nOu remplir le fichier", " CHARGEMENT", MB_OK | MB_ICONWARNING);
        ...
        case 10:
            if (MessageBox(hwnd, "       Vous voulez enregistrer la liste ?", " ENREGISTREMENT", MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
            {
                Enregistrement();
            }
            else if (MessageBox(hwnd, "       Vous voulez quitter ?", " QUITTER", MB_YESNO | MB_ICONQUESTION) == IDYES)
            {
                DestroyWindow(hwnd);
                system("cls");
            }
