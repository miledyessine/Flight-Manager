...
void recherche_ident()
{
    ...
    if (recherche_av(ch))
    {
        ...
        MessageBox(hwnd, "       Cette avion n'existe pas", " RECHERCHE", MB_OK | MB_ICONHAND);
}
...
void recherche_comp()
{
    ...
    if (recherche_cp(ch))
    {
        ...
        MessageBox(hwnd, "       Cette compagnie n'existe pas", " RECHERCHE", MB_OK | MB_ICONHAND);
}
void recherche_deco()
{
    char ch[9];
    ...
    if (recherche_de(ch))
    {
        ...
        MessageBox(hwnd, "       Cette heure n'existe pas", " RECHERCHE", MB_OK | MB_ICONHAND);
}
...
unsigned verif_comp(char *ch)
{
    if (strstr(ch, "  ") || ch[0] == ' ' || ch[strlen(ch) - 1] == ' ')
        return 0;
    for (int i = 0; ch[i] != '\0'; i++)
    {
...
void insere_tete_avion()
{
    ...
    } while (!verif_comp(aux->avion->compagnie->nom));
    do
    {
        gotoxy(37, 16);
        printf("                    ");
...
void suppression_av_donne()
{
    ...
    if (recherche_av(info))
    {
        ...
        MessageBox(hwnd, "       Cette avion n'existe pas", " SUPPRIME", MB_OK | MB_ICONHAND);
}
void suppression_comp_donne()
{
    ...
    if (recherche_cp(ch))
    {
        ...
        MessageBox(hwnd, "       Compagnie n'existe pas", " SUPPRIME", MB_OK | MB_ICONHAND);
}
...
void modifier_ident()
{
    ...
    if (recherche_av(ch))
    {
        ...
        MessageBox(hwnd, "     Cette avion n'existe pas", " MODIFICATION", MB_OK | MB_ICONHAND);
}
...
void modifier_comp()
{
    ...
    do
    {
        printf(" Donner le nouveau nom du compagnie : ");
        fflush(stdin);
        gets(ch1);
    } while (!verif_comp(ch1));
    element *tmp = piste.head;
    while (tmp && strcmp(ch, tmp->avion->identifiant))
        tmp = tmp->suivant;
    if (tmp)
...
void affiche_avion_donne()
{
    ...
    if (recherche_av(info))
    {
        ...
        MessageBox(hwnd, "       Cette avion n'existe pas", " AFFICHE", MB_OK | MB_ICONHAND);
    printf("\n");
}
...
void affiche_comp_donne()
{
    ...
    if (recherche_cp(ch))
    {
        ...
        MessageBox(hwnd, "       Cette compagnie n'existe pas", " AFFICHE", MB_OK | MB_ICONHAND);
    printf("\n");
}
